...
from reader import Reader, NotEnoughDataException
...
class ModelTrainer():
    ...
    def _get_base_config(self):
        ...
        config = {
            'prediction_size' : 1,
            'num_predictions' : 3, #number of predictions that will be made for each window
    ...
    def train(self):
        ...
        test_predictions = [[] for _ in range(config['num_predictions'])]
        ...
        with tf.Graph().as_default():
            ...
            with tf.Session(config=tf.ConfigProto(allow_soft_placement=False)) as session:
                ...
                while reader.next_window():
                    ...
                    npredictions = [] 
                    for pred_num, tp in enumerate(test_predictions): 
                        try:
                            test_ii, epoch_size = reader.get_iterator_initializer(config['batch_size'], config['num_steps'], predictions=npredictions)
                            test_mse, predictions = self._run_epoch(session, mtest, test_ii, epoch_size)
                            tp.append(predictions[-1])
                            npredictions.append(predictions[-1])
                            train_writer.add_summary(TensorflowUtils.summary_value('Test Loss {}'.format(pred_num + 1), test_mse), global_step)
        ...
        for pred_num, tp in enumerate(test_predictions):        
            evaluator = Evaluator(reader, tp, -1, global_step)
            sp = os.path.join(config['save_path'], 'prediction{}'.format(pred_num + 1))
            evaluator_sm.pickle(evaluator, sp, evaluator.absolute_mean_error(), PickleAction.BEST)
            config_sm.pickle(config, sp, evaluator.absolute_mean_error(), PickleAction.BEST)
            print()
            print("Prediction {} Absolute Mean Error: {:.2f} Relative Mean Error: {:.2f}%".format(pred_num + 1, evaluator.absolute_mean_error(), evaluator.relative_mean_error()))
            print()
