...
class BaseEvaluatorActionMenu(ActionMenu):
    ...
    def _perform_action(self, action, command):
        ...
        if action == 1:
            ...
            self._actor.plot_target_vs_predicted(feature_pos, evals=evals, scaled=scaled, tail=tail, prediction_indexes=prediction_indexes)
        elif action == 4:
            self._actor.plot_errors(feature_pos, evals=evals, scaled=scaled, prediction_index)
        elif action == 5:
            self._actor.plot_absolute_errors(feature_pos, evals=evals, scaled=scaled, prediction_indexes)
        elif action == 6:
            self._actor.plot_relative_errors(feature_pos, evals=evals, scaled=scaled, prediction_indexes)
        elif action == 7:
            print(self._actor.absolute_mean_error_str(feature_pos, scaled=scaled, evals=evals, prediction_indexes))
        elif action == 8:
            print(self._actor.relative_mean_error_str(feature_pos, scaled=scaled, evals=evals, prediction_indexes))
        else:
